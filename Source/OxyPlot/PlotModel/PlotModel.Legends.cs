...
namespace OxyPlot
{
    ...
    public partial class PlotModel
    {
        ...
        private OxySize RenderOrMeasureLegends(IRenderContext rc, OxyRect rect, bool measureOnly = false)
        {
            ...
            IEnumerable<Series.Series> items;
            if (!this.DisplayCustomLegend)
            {
                items = this.LegendItemOrder == LegendItemOrder.Reverse ? this.VisibleSeries.Reverse() : this.VisibleSeries;
            }
            else
            {
                var customVisibleSeies = this.CustomSeries.Where(s => s.IsVisible);
                items = this.LegendItemOrder == LegendItemOrder.Reverse ? customVisibleSeies.Reverse() : customVisibleSeies; 
            }
